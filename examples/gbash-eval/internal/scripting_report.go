...
func printScriptingTerminalReport(w io.Writer, report ScriptingEvalReport) {
	...
	if report.Baseline {
		fmt.Fprintln(w, "  Mode: baseline (individual tools)")
	} else {
		fmt.Fprintln(w, "  Mode: scripted (bash orchestration tool)")
	}
